"""
TEMS Recent Diagnostics Reporter (S50 — TGL #128 hook-level enforcement, α layer)
====================================================================================
SessionStart 시점에 `tems_diagnostics.jsonl` 의 최근 N시간 내 `*_failure` 이벤트를
stdout 으로 표시. 위상군이 자기보고 작성 전 production failure 를 누락하는
S49-style 메타-결함을 차단하기 위한 가시화 레이어.

Read-only — DB 미접속, jsonl 만 read. 신규 모듈이므로 TGL #102 백업 의무 비대상.
...
# .resolve() canonical pattern (S49 P0 — cwd 비의존)
...
def format_human(failures: list[dict], hours: int, stale_pending: list[dict] | None = None) -> str:
    ...
    lines.append(
        f"  → 핸드오버 작성 시 위 failure 의 후속 처치/원인 누락 금지. "
