...
- 사용자가 명시적으로 권한을 승인한 도구 호출을 "차단"하는 것은 무거운 결정.
  → 기본 severity는 warning. critical은 종일군이 명시 지시한 규칙만.
...
stdout:
  ...
  - 경고 시: <tgl-tool-alert>...</tgl-tool-alert> 텍스트 (위상군 컨텍스트에 주입)
  - 무매칭 시: 아무것도 출력하지 않음
"""
...
def main():
    ...
    if blocking_hits:
        ...
        reason_lines = [
            ...
            rule["rule_body"][:400],
            "이 차단이 잘못되었다면 종일군 확인 후 규칙을 조정하거나 severity를 warning으로 낮추세요.",
        ]
        payload = {
            "hookSpecificOutput": {
